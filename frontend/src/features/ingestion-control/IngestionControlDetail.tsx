...
import { ControlDestinationsManager } from "@/features/ingestion-control/ControlDestinationsManager";
...
export function IngestionControlDetail({ item, onClose }: { item: IngestionControl | null; onClose: () => void }) {
  const { can } = useAuth();
  const canWrite = can("ingest:control:write");
  return (
    <Modal
      open={!!item}
      onClose={onClose}
    ...
    >
      {item && (
        <dl className="grid grid-cols-2 gap-x-4 gap-y-3">
          ...
          <SectionTitle>Destinos da carga</SectionTitle>
          <ControlDestinationsManager controlId={item.id} controlTable={item.nome_tabela?.split(".").pop() ?? ""} canWrite={canWrite} />
