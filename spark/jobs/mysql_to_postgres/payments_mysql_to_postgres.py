...
def main() -> int:
    ...
    df = (
        ...
        # MySQL ENUM columns can come back space-padded to the widest label via JDBC; trim
        # so enum validation and the ::enum cast match exactly.
        .withColumn("payment_method", F.trim(F.col("payment_method").cast("string")))
        ...
        .withColumn("payment_status", F.trim(F.col("payment_status").cast("string")))
...
def _prepare_target(host, port, db, user, password, schema, staging_fqn) -> None:
    """Ensure schema, enum types, final table and staging exist (all idempotent).

    Never drops or recreates an existing final table (CREATE TABLE IF NOT EXISTS).
    """
    method_values = ", ".join(f"'{v}'" for v in PAYMENT_METHODS)
    status_values = ", ".join(f"'{v}'" for v in PAYMENT_STATUSES)
    with _pg_connect(host, port, db, user, password) as conn:
        with conn.cursor() as cur:
            cur.execute(f'CREATE SCHEMA IF NOT EXISTS "{schema}"')
            # Enum types (Postgres has no CREATE TYPE IF NOT EXISTS -> guard with DO block).
            cur.execute(
                f"DO $$ BEGIN "
                f"IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid=t.typnamespace "
                f"WHERE t.typname='payment_method_enum' AND n.nspname='{schema}') THEN "
                f"CREATE TYPE {schema}.payment_method_enum AS ENUM ({method_values}); END IF; END $$;"
            )
            cur.execute(
                f"DO $$ BEGIN "
                f"IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid=t.typnamespace "
                f"WHERE t.typname='payment_status_enum' AND n.nspname='{schema}') THEN "
                f"CREATE TYPE {schema}.payment_status_enum AS ENUM ({status_values}); END IF; END $$;"
            )
            # Final business table (kept if it already exists).
            cur.execute(
                f"""
                CREATE TABLE IF NOT EXISTS {schema}.payments (
                    ...
                    payment_status {schema}.payment_status_enum NOT NULL DEFAULT 'PENDING',
                    paid_at TIMESTAMP,
                    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
                )
                """
            )
            # Staging (VARCHAR enums) — created fresh and truncated each run.
